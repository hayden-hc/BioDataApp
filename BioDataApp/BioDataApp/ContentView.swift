...
struct SyncView: View {
    ...
    @State private var apiRecommendations: [String] = []
    @State private var usedAPI: Bool = false
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 32) {
                    ...
                    // Mood (for API score)
                    VStack(spacing: 6) {
                        Text("Mood (0–10) for today's score")
                    ...
                    if !apiRecommendations.isEmpty {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ForEach(apiRecommendations, id: \.self) { rec in
                                    Text("• \(rec)")
                                        .font(.system(size: 12))
    ...
    private func triggerSync() {
        ...
        usedAPI = false
        apiRecommendations = []
        HealthBridge.shared.syncToday { steps, rhr, exercise, sleep in
            DispatchQueue.main.async {
                refreshStats()
                // If profile is set, send to local TallyWell API (Python model)
                if AppSettings.hasProfile {
                    let req = ScoreRequest(
                        userId: TallyWellAPI.shared.userId,
                        profile: AppSettings.profile,
                        steps: Double(steps),
                        exerciseMinutes: Double(exercise),
                        sleepHours: sleep,
                        mood: mood,
                        restingHr: rhr
                    )
                    Task {
                        do {
                            let response = try await TallyWellAPI.shared.postScore(req)
                            await MainActor.run {
                                todayScore = response.score
                                apiRecommendations = response.recommendations
                                usedAPI = true
                            }
                        } catch {
                            await MainActor.run {
...
// ─────────────────────────────────────────────
// MARK: - Settings (API URL + profile for TallyWell)
// ─────────────────────────────────────────────
...
struct SettingsView: View {
    ...
    var body: some View {
                ...
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        Text("TallyWell API (run on your Mac)")
                            ...
                            .foregroundColor(Color(red: 0.79, green: 0.66, blue: 0.30))
                        Text("Simulator: use localhost. Device: use your Mac's IP, e.g. http://192.168.1.42:5001")
                            .font(.system(size: 11))
                            .foregroundColor(Color(white: 0.5))
        ...
        .onAppear {
            ...
            height = "\(Int(AppSettings.heightCm))"
            weight = "\(Int(AppSettings.weightKg))"
            age = "\(AppSettings.age)"
